// routes/library/moderation/keep_under_review.js — HakMun API (v0.12)
// POST /v1/library/keep-under-review (root-admin-only)
...
router.post("/v1/library/keep-under-review", requireSession, requireRootAdmin, async (req, res) => {
  const rid = req._rid;
    ...
    try {
      ...
      if (!rqRow?.id) {
        ...
        return res.status(409).json({
          error: "cannot keep_under_review: missing unresolved review queue row"
        });
      }
